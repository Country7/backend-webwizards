...
# Реализация RESTful HTTP API в Go с помощью Gin (часть 2.1)
...
существовании ключей, предназначенных специально для работы с конфликтами:
...
Если мы хотим запушить нашу ветку, то для этого нужно написать:
    ...
    $ git push -u origin <branch>   // -u для запоминания (отслеживания ветки)
